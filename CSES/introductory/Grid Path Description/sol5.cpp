...
// if we have formed something of this form
//..X
//XoX
//.X
// unless o is the final square, impossible
// as it forms a cage where to visit o, it must terminate at o
bool has_formed_snake(int row, int col)
{
    // up and snake cage to left
    if (!is_valid(row + 1, col) && !is_valid(row + 2, col - 1) && !is_valid(row + 1, col - 2) && is_valid(row + 1, col - 1) && !(row + 1 == GRID_SIZE - 1 && col - 1 == 0))
    {
        return true;
    }
    // up and snake cage to right
    if (!is_valid(row + 1, col) && !is_valid(row + 2, col + 1) && !is_valid(row + 1, col + 2) && is_valid(row + 1, col + 1) && !(row + 1 == GRID_SIZE - 1 && col + 1 == 0))
    {
        return true;
    }
    // down and snake cage to left
    if (!is_valid(row - 1, col) && !is_valid(row - 2, col - 1) && !is_valid(row - 1, col - 2) && is_valid(row - 1, col - 1) && !(row - 1 == GRID_SIZE - 1 && col - 1 == 0))
    {
        return true;
    }
    // down and snake cage to right
    if (!is_valid(row - 1, col) && !is_valid(row - 2, col + 1) && !is_valid(row - 1, col + 2) && is_valid(row - 1, col + 1) && !(row - 1 == GRID_SIZE - 1 && col + 1 == 0))
    {
        return true;
    }
    // left and snake cage up
    if (!is_valid(row, col - 1) && !is_valid(row - 1, col - 2) && !is_valid(row - 2, col - 1) && is_valid(row - 1, col - 1) && !(row - 1 == GRID_SIZE - 1 && col - 1 == 0))
    {
        return true;
    }
    // left and snake cage down
    if (!is_valid(row, col - 1) && !is_valid(row + 1, col - 2) && !is_valid(row + 2, col - 1) && is_valid(row + 1, col - 1) && !(row + 1 == GRID_SIZE - 1 && col - 1 == 0))
    {
        return true;
    }
    // right and snake cage up
    if (!is_valid(row, col + 1) && !is_valid(row - 1, col + 2) && !is_valid(row - 2, col + 1) && is_valid(row - 1, col + 1) && !(row - 1 == GRID_SIZE - 1 && col + 1 == 0))
    {
        return true;
    }
    // right and snake cage down
    if (!is_valid(row, col + 1) && !is_valid(row + 1, col + 2) && !is_valid(row + 2, col + 1) && is_valid(row + 1, col + 1) && !(row + 1 == GRID_SIZE - 1 && col + 1 == 0))
    {
        return true;
    }
...
void dfs(int row, int col, int depth)
{
    ...
    if (has_formed_snake(row, col)) return;
